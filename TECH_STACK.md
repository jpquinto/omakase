...
- **TanStack Query** for data fetching via orchestrator REST API (projects, features, agents)
...
- RBAC roles: `admin`, `developer`, `viewer` (stored in DynamoDB `users` table)
- M2M tokens for orchestrator-to-API communication
...
**Location:** `packages/dynamodb/`

AWS DynamoDB data access layer with repository pattern.

### Schema (6 tables)

| Table | Purpose |
|---|---|
| `users` | Auth0 users with roles (GSI on `auth0Id`, `email`) |
...
- `src/repositories/features.ts` -- Feature status transitions, ready-feature query (checks dependency resolution)
...
**Deployed:** DynamoDB tables are provisioned via AWS CDK. Table prefix controlled by `DYNAMODB_TABLE_PREFIX` environment variable.
...
Pure TypeScript type definitions that serve as the **shared type system**. Exports types (`Feature`, `Project`, `Agent`, `User`, `Ticket`, `AgentRun`) that other packages import for type safety.
...
| `feature-watcher.ts` | Polls DynamoDB every 30s for pending features across all projects |
...
6. On success: feature marked `passing` in DynamoDB, PR created on GitHub
...
- `PROJECT_ID` -- DynamoDB project reference
- `DYNAMODB_TABLE_PREFIX` -- For DynamoDB table access
...
|---|---|---|
| `ci.yml` | Push/PR to `main` | Typecheck all packages, lint web app, run Playwright E2E |
...
- Bidirectional sync: Linear issues <-> DynamoDB features
...
Linear ticket created
  -> Webhook syncs to DynamoDB "tickets" table
    -> Feature created in DynamoDB "features" table
      -> FeatureWatcher picks up ready feature
        -> AgentPipeline launches on ECS Fargate:
          ...
          4. Tester (verifies)
        -> Feature marked "passing" in DynamoDB
        -> PR created on GitHub
          -> Dashboard updates via REST API polling
```
